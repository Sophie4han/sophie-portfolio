...
const production = process.env.STEP09_PRODUCTION_URL ?? 'http://localhost:3030';
const development = process.env.STEP09_DEVELOPMENT_URL ?? 'http://localhost:3022';
const debuggerUrl = process.env.STEP09_DEBUGGER_URL ?? 'http://127.0.0.1:9229';
...
try {
  ...
  await send('Page.navigate', { url:`${development}/dev/fitmate-island` });
  await waitFor(`document.querySelector('[data-camera-preset]')?.dataset.cameraPreset==='UNDERSTAND_FOCUS' && [...document.images].every(i=>i.complete&&i.naturalWidth>0)`);
  await sleep(500);
  results.desktopFitMateFocus = await measure();
  assertScene(results.desktopFitMateFocus);
  await screenshot('desktop-fitmate-focus');
  await run(`document.querySelector('button[data-island="fitmate"]').focus()`);
  results.focusOutline = await run(`({style:getComputedStyle(document.activeElement).outlineStyle,width:getComputedStyle(document.activeElement).outlineWidth})`);
  await screenshot('desktop-fitmate-keyboard-focus');
  results.landing = await run(`(() => {
    const img=document.querySelector('[data-destination="fitmate"] img'); const r=img.getBoundingClientRect();
    const x=r.x+1145/1920*r.width,y=r.y+845/1080*r.height;
    document.querySelector('[data-chooni-intent="world-idle"]').style.visibility='hidden';
    const marker=document.createElement('div'); marker.style.cssText='position:fixed;z-index:100;pointer-events:none;width:14px;height:14px;border:1px solid #933;transform:translate(-50%,-50%);border-radius:50%;left:'+x+'px;top:'+y+'px';document.body.append(marker);
    return {logical:{x:985,y:755},screen:{x,y},method:'Fixed landing coordinate projected through actual raster transform. Existing placeholder hidden only for this diagnostic.'};
  })()`);
  await screenshot('landing-destination');
  const productionFixture = await fetch(`${production}/dev/fitmate-island`);
