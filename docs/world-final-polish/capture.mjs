...
const output = new URL((process.env.CAPTURE_PHASE ?? 'before')+'/', import.meta.url);
...
try {
  ...
  for (const [name, width, height] of [['desktop', 1440, 900], ['mobile', 390, 844]]) {
    ...
    if (name === 'desktop') {
      ...
      await run(`document.querySelector('button[data-island="harubareun"]').click()`);
      await waitFor(`document.querySelector('[data-camera-preset]')?.dataset.cameraPreset==='BUILD_FOCUS'`);
      await sleep(1000);
      results.harubareunFocus = await measure();
      await screenshot('harubareun-focus');
    }
  }
  for(const [id,route,preset] of [['invader','invader-island','ITERATE_FOCUS'],['fitmate','fitmate-island','UNDERSTAND_FOCUS']]) {
    await send('Emulation.setDeviceMetricsOverride', {width:1440,height:900,deviceScaleFactor:1,mobile:false});
    await send('Page.navigate', {url:'http://localhost:3022/dev/'+route});
    await waitFor(`document.querySelector('[data-camera-preset]')?.dataset.cameraPreset==='${preset}' && [...document.images].every(i=>i.complete&&i.naturalWidth>0)`);
    await sleep(800);results[id+'Focus']=await measure();await screenshot(id+'-focus');
  }
