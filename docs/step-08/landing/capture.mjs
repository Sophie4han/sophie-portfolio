...
const production = process.env.STEP08_PRODUCTION_URL ?? 'http://localhost:3023';
const development = process.env.STEP08_DEVELOPMENT_URL ?? 'http://localhost:3022';
const debuggerUrl = process.env.STEP08_DEBUGGER_URL ?? 'http://127.0.0.1:9229';
const output = new URL('./', import.meta.url);
const metadata = JSON.parse(await fs.readFile(new URL('../asset-validation.json', output), 'utf8'));
metadata.production.alphaBounds = JSON.parse(await fs.readFile(new URL('validation.json', output), 'utf8')).alphaBounds;
...
const measure = () => run(`(() => {
  ...
  const definitions = ${JSON.stringify({ harubareun: metadata.master.alphaBounds, 'project-02': metadata.production.alphaBounds })};
...
const assertScene = result => {
  ...
  for (const island of result.islands.filter(island => island.visibleArt)) {
    ...
    if (result.focused === 'false' || island.id === 'project-02') {
...
try {
  ...
  const results = { method: 'Production Overview via normal Prologue flow. INVADER Focus via development-only isolated WorldScene fixture. No unlock-rule or app timer overrides.' };
  for (const [name, width, height] of [['desktop', 1440, 900]]) {
    ...
    assert.equal(results[`${name}Overview`].targets.find(t=>t.id==='project-02').disabled, 'true');
    await screenshot(`${name}-overview`);
    await run(`document.querySelector('button[data-island="project-02"]').click()`);
    await sleep(500);
    ...
    if (name === 'desktop') {
      ...
      assert.equal(results.buildHover.islands.find(i=>i.id==='project-02').filter, 'none');
      await send('Input.dispatchMouseEvent', { type:'mouseMoved', x:0, y:0 });
    }
  }
  await send('Emulation.setDeviceMetricsOverride', { width:1440, height:900, deviceScaleFactor:1, mobile:false });
  await send('Page.navigate', { url:`${development}/dev/invader-island` });
  await waitFor(`document.querySelector('[data-camera-preset]')?.dataset.cameraPreset==='ITERATE_FOCUS' && [...document.images].every(i=>i.complete&&i.naturalWidth>0)`);
  await sleep(500);
  results.desktopInvaderFocus = await measure();
  assertScene(results.desktopInvaderFocus);
  await screenshot('desktop-invader-focus');
  // Diagnostic overlay only: hide the existing circle placeholder to expose ground.
  results.landing = await run(`(() => {
    const img=document.querySelector('[data-destination="project-02"] img');
    const r=img.getBoundingClientRect();
    const x=r.x+1195/1920*r.width,y=r.y+465/1080*r.height;
    document.querySelector('[data-chooni-intent="world-idle"]').style.visibility='hidden';
    const marker=document.createElement('div');
    marker.style.cssText='position:fixed;z-index:100;pointer-events:none;width:14px;height:14px;border:1px solid #933;transform:translate(-50%,-50%);border-radius:50%;left:'+x+'px;top:'+y+'px';
    marker.title='Fixed landing destination (1035,375)';document.body.append(marker);
    return {logical:{x:1035,y:375},screen:{x,y},method:'Unchanged manifest anchor projected through actual image transform. Circle placeholder hidden only in diagnostic screenshot.'};
  })()`);
  await screenshot('landing-destination');
  const productionFixture = await fetch(`${production}/dev/invader-island`);
  results.productionFixtureStatus = productionFixture.status;
  assert.equal(productionFixture.status, 404);
